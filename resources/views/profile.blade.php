<x-app-layout>
    @php
        $rating = round($user->reviewsReceived()->avg('rating') ?? 0, 1);
    ...
    <div class="container py-5">
        <div class="card border-0 shadow-lg rounded-5 overflow-hidden mb-5">
            <div style="height:260px; position:relative; overflow:hidden;">
                @if($user->banner)
                    <img src="{{ asset('storage/'.$user->banner) }}" style="width:100%; height:100%; object-fit:cover;">
                @else
                ...
                <div class="row align-items-end gx-5">
                    ...
                    <div class="col">
                        ...
                        <div class="d-flex gap-2 flex-wrap mb-3">
                            ...
                            <span class="badge bg-dark px-3 py-2">Member since {{ $user->created_at->format('Y') }}</span>
                            @if($user->is_admin)
                                <span class="badge bg-danger px-3 py-2">Administrator</span>
                            @endif
                        </div>

                        <p class="text-secondary mb-1">📍 {{ $user->city ?? 'Location not set' }}</p>
                        <p class="text-secondary">{{ $user->bio ?? 'No bio yet.' }}</p>
                    </div>

                    <div class="col-lg-auto">
                        @if(Auth::id() != $user->id)
                            <a href="/messages/start/{{ $user->id }}" class="btn btn-primary btn-lg rounded-4 px-4 shadow-sm">✉ Direct Message</a>
                        @endif
                    </div>
                </div>
            </div>
        ...
        <div class="row g-4 justify-content-center mb-4">
            <div class="col-md-4">
                <a href="/inventory/{{ $user->id }}" class="text-decoration-none d-block h-100">
                    <div class="card shadow-sm border-0 rounded-5 text-center hover-card h-100">
                        <div class="card-body py-4">
                            <h1 class="fw-bold text-primary">{{ $items }}</h1>
            ...
            <div class="col-md-4">
                <div class="card shadow-sm border-0 rounded-5 text-center h-100">
                    <div class="card-body py-4">
                        <h1 class="fw-bold text-success">{{ $rentals }}</h1>
            ...
            <div class="col-md-4">
                <a href="/users/{{ $user->id }}/reviews" class="text-decoration-none d-block h-100">
                    <div class="card shadow-sm border-0 rounded-5 text-center hover-card h-100">
                        <div class="card-body py-4">
            ...
            <div class="col-lg-7">
                <div class="card shadow-sm border-0 rounded-5 h-100">
                    <div class="card-body p-4">
                        <h4>About User</h4>
                        <hr>
                        <p class="mb-0">{{ $user->bio ?? 'No information provided.' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-5">
                <div class="card shadow-sm border-0 rounded-5 h-100">
                    <div class="card-body p-4">
                        ...
                        <hr>
                        <div class="mb-3">📦 {{ $items }} active listings</div>
                        <div class="mb-3">🤝 {{ $completedRentals }} completed rentals</div>
