...
Route::get('/dashboard', function() {
    ...
    return view('dashboard', compact('itemsCount', 'activeRentals', 'returnedRentals', 'cancelledRentals'));
})->middleware(['auth', 'verified', 'ban'])->name('dashboard');
...
Route::middleware(['auth', 'ban'])->group(function() {
    ...
    Route::get('/inventory/{user}', [ProfileController::class, 'userItems'])->name('profile.items');
    Route::get('/users/{user}/reviews', [ProfileController::class, 'userReviews'])->name('profile.reviews');
    ...
    Route::get('/inventory', function() {
        $user = Auth::user();
        $items = $user->items()->with('category')->latest()->get();
    ...
    Route::get('/rentals', function() {
        $user = Auth::user();
        $activeRentals = $user->rentals()->where('status', 'active')->with('item')->latest()->get();
        $history = $user->rentals()->whereIn('status', ['returned', 'cancelled'])->with('item')->latest()->get();
        return view('rentals', compact('activeRentals', 'history'));
    })->name('rentals');
    Route::post('/rentals', [RentalController::class, 'store'])->name('rentals.store');
    Route::patch('/rentals/{rental}/return', [RentalController::class, 'returnRental']);
    Route::patch('/rentals/{rental}/cancel', [RentalController::class, 'cancelRental']);
    Route::get('/rent/{item}', [RentalController::class, 'create'])->name('rentals.create');

    // REVIEWS
    Route::get('/reviews/{rental}', [ReviewController::class, 'create'])->name('reviews.create');
    ...
    Route::get('/messages/start/{user}', [MessageController::class, 'start']);
...
// ADMIN
Route::middleware(['auth', 'ban', 'admin'])->prefix('admin')->group(function() {
