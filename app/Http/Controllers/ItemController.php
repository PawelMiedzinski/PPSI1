...
class ItemController extends Controller implements HasMiddleware
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->filled('search')) {
            $query->where(fn($q) => $q->where('title', 'like', "%{$request->search}%")->orWhere('description', 'like', "%{$request->search}%"));
        }
        ...
        switch ($request->sort) {
            case 'price_low': $query->orderBy('price_per_day'); break;
            case 'price_high': $query->orderBy('price_per_day', 'desc'); break;
    ...
    {
        if ($item->owner_id !== Auth::id()) abort(403, 'Brak uprawnień.');
        $categories = Category::all();
    ...
    {
        if ($item->owner_id !== Auth::id()) abort(403, 'Unauthorized access.');
        ...
        if ($request->hasFile('image')) {
            if ($item->image && !str_contains($item->image, 'picsum.photos')) {
    ...
    {
        if ($item->owner_id !== Auth::id()) abort(403, 'No access.');
